...
NewPing sonar_Carr9HNorte   (TRIGGER_PIN_1, ECHO_PIN_1, MAX_DISTANCE);// sensor 1
NewPing sonar_Carr9HSur     (TRIGGER_PIN_2, ECHO_PIN_2, MAX_DISTANCE);// sensor 2
NewPing sonar_Carr6HNorte   (TRIGGER_PIN_3, ECHO_PIN_3, MAX_DISTANCE);// sensor 3
NewPing sonar_Carr6HSur     (TRIGGER_PIN_4, ECHO_PIN_4, MAX_DISTANCE);// sensor 4
...
int contarCarros(int numSensor){
  ...
  int cuenta =0;        //Variable usada para el retorno 
  int cuenta_Temp = 0;  //Variable dentro del for ..contador que cuanta el numero de carros
  int cuenta_ejec = 0;  //Contador del numero de ciclos del for
  int distancia = 0;    //Distancia usada para el analisis 
  int distancia1 = 0;   //Distancia temporal ..elimina en el caso de que la distancia sea cero . porque si es cero cuenta un carro
  int bandera = 0;      //Usada como flanco de subida y bajada
    
  for (int i=0; i <= 100; i++){
    
    if(numSensor == 1){
      distancia1 =sonar_Carr9HNorte.ping_cm();
    }else if(numSensor == 2){
      distancia1 =sonar_Carr9HSur.ping_cm();
    }else if(numSensor == 3){
      distancia1 =sonar_Carr6HNorte.ping_cm();
    }else{
      distancia1 =sonar_Carr6HSur.ping_cm();
    }
...
void loop() {
      ...
      Serial.print("Carros 9Norte:");
      Serial.print(Num_Carr9HNorte);
      Serial.println("\n");
      Serial.print("Carros 9Sur:");
      Serial.print(Num_Carr9HSur);
      Serial.println("\n");
      Serial.print("Carros 6Norte:");
      Serial.print(Num_Carr6HNorte);
      Serial.println("\n");
      Serial.print("Carros 6Sur:");
      Serial.print(Num_Carr6HSur);
      Serial.println("\n");
